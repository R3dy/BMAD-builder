...
|-----------|-------------------|
| PR #1, #2, or #3 overall in Phase 4 | Human review required — always |
| Story title or technical tasks contain "webhook" | Human review required — always, regardless of PR count |
...
Any story whose title or technical task list contains the word "webhook" requires human review of the PR regardless of the current PR count. The orchestrator checks for this keyword when evaluating the PR review rule after each validation PASS.

**Security failure override:**
Any security check failure in a validation report produces a verdict of ESCALATE, not FAIL. Security failures never go back to the worker for retry — they always go to the product owner.

**Human-only criterion override:**
Any acceptance criterion that requires visual inspection, browser testing, or UX judgment produces a verdict of ESCALATE. The validator must list the specific human-only criteria in the escalation reason. The orchestrator pauses and the product owner manually verifies before the orchestrator continues.
...
| `"skip story N.N"` | Mark story N.N as Done with note "manually skipped", continue loop |
...
| `✅` | Done | Merged to main |
| `🚫` | Blocked | Escalated — awaiting your decision |
